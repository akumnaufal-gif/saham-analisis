    ...
    with st.spinner("Mengambil & menganalisis data..."):
        ...
        if mode in ["🔍 Screen Semua Saham", "📋 Manual Ticker"]:
            if mode == "📋 Manual Ticker":
                ...
                popular = ["BBCA","BBRI","BMRI","BBNI","TLKM","ASII","ADRO","GOTO","AMRT","UNVR","BRIS","BTPS","CPIN","KLBF","MDKA"]
            ...
            ticker_bt = st.text_input("Masukkan Ticker", "BBCA")
            if st.button("Jalankan Backtest"):
                ...
                if len(hist) > 100:
                    ...
                    col1.metric("Strategi Return", f"{total_return:.1%}")
                    col2.metric("Buy & Hold", f"{buy_hold:.1%}")
                    col3.metric("Jumlah Trade", len(hist[hist['Signal']]))
                    st.line_chart(hist[['Close', 'Strategy']].cumsum())

        elif mode == "📈 Heatmap Sektor":
            st.subheader("📊 Heatmap Performa Sektor (3 Bulan)")
            st.info("Sedang dalam pengembangan...")

st.caption("Data dari Yahoo Finance • Score adalah gabungan indikator • Bukan rekomendasi trading")
